(ns hypercrud.server.test.api
  (:require [clojure.test :refer [deftest is use-fixtures]]
            [datomic.api :as d]
            [hypercrud.server.api :as api]
            [hypercrud.server.test.fixtures :as fixtures]))
...
(deftest branch-once []
  (let [conn (d/connect fixtures/test-uri)
        ...
        get-secure-db-with (api/build-get-secure-db-with staged-branches (atom {}) local-basis)
...
(deftest branch-once-stale []
  (let [conn (d/connect fixtures/test-uri)
        ...
        get-secure-db-with (api/build-get-secure-db-with staged-branches (atom {}) local-basis)]
    (is (thrown-with-msg? RuntimeException (re-pattern api/ERROR-BRANCH-PAST)
