...
(defn nested-links-for-field
  ([parent-fiddle dbname schema field parent-path parent-has-id]
   (let [{segment ::field/path-segment} field
     ...
     (let [id (field/identity-segment? field)
           ref (and (not id) (::field/data-has-id? field))
           id-under-ref (and id parent-has-id)
           ref-under-ref (and ref parent-has-id)
           id-under-FindColl (and id (not parent-has-id)) #_"Insufficient condition to distinguish from scalar and entity"] ; count path
       (->> [(if ref (hf-edit child-path dbname))
             ;(if (and id (seq parent-path)) (hf-edit parent-path dbname)) ; parent ref already did it
             (if ref (hf-detach child-path))
             ...
             (if id-under-FindColl (hf-new child-path dbname))]
            (concat
              (if-not (field/children-identity-only? field)
                (->> (::field/children field)
                     (mapcat #(nested-links-for-field parent-fiddle dbname schema % child-path (::field/data-has-id? field))))))
            (remove nil?)
       ...
       (mapcat (fn [{:keys [::field/path-segment] :as field}]
                 (let [dbname (str (::field/source-symbol field))
                   ...
                   (cond->> (nested-links-for-field parent-fiddle dbname schema field [] false)
                     #_(not= :entity (:fiddle/type parent-fiddle))
...
(defn console-links
  ...
  (let [missing-fe-wrapper (::field/source-symbol field)]
    (if missing-fe-wrapper
      (system-links-impl parent-fiddle [field] schemas)     ; wrapper for FindColl, FindTuple, etc? weird.
