...
(def read-handlers
  (atom {
   ...
   "sorted-map" (t/read-handler #(into (sorted-map) %)) ; Sorted Map Decoding
   }))
...
(def write-handlers
  (atom {
   ...
   #?@(:clj [clojure.lang.PersistentTreeMap (t/write-handler (constantly "sorted-map") (fn [v] (into {} v)))]) ; Sorted Map Encoding
   }))
