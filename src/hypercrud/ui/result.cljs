...
(defn result-renderer [result _ _ ctx]
...
(defn view [result ordered-fes links ctx]
  ;(:anchors ctx)
  (let [ctx (assoc ctx :ordered-fes ordered-fes :links links)
        {index-inline-links true index-links false} (->> (link/links-lookup' links [])
