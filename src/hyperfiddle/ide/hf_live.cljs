...
(defn docs-embed [attrs ctx-real class & {:keys [embed-mode]}]
  (let [ctx-real (dissoc ctx-real :user-renderer)           ; this needs to not escape this level; inline links can't ever get it
        ...
        controls
        {:fiddle/pull (r/partial control-with-unders (fragment [:span.schema "schema: " (fiddle-src/schema-links ctx)]))
         :fiddle/query (r/partial control-with-unders (fragment [:span.schema "schema: " (fiddle-src/schema-links ctx)]))
