...
(defn- full-html [build env-analytics-enabled rt client-params]
  [:html {:lang "en"}
   [:head
    ...
    [:link {:rel "stylesheet" :href (domain/api-path-for (runtime/domain rt) :static-resource :build build :resource-name "browser.css")}]
...
(defn bootstrap-html-cmp [env domain io route user-id]
  (let [initial-state {::runtime/auto-transact (data/map-values
        ...
        rt (->RT domain io (r/atom (state/initialize initial-state)))]
