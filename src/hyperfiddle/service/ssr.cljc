...
(defn- full-html [build env-analytics-enabled rt client-params]
  [:html {:lang "en"}
   [:head
    [:title "Hyperfiddle" #_(str (first @(runtime/state rt [::runtime/partitions foundation/root-branch :route])))]
    ...
    [:link {:rel "stylesheet" :href (domain/api-path-for (runtime/domain rt) :static-resource :build build :resource-name "styles.css")}]
