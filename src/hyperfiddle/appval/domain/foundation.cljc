...
(defn local-basis [user-local-basis global-basis domain route]
  (concat
    (:domain global-basis)
    (user-local-basis global-basis domain route)))

; Foundation needs to give domain, and thats it i guess
(defn api [user-api-fn hyperfiddle-hostname hostname branch state-val]
